...
- **Swipe-back is interactive with a destination peek** (`useSwipeBack`): an edge pan (starts within 30px of the left edge) drags the board with the finger via a direct `translateX`, while the **Journals screen parallaxes in behind it** — the board mounts `<Home/>` as a peek layer (`fixed inset-0 z-0`) only while dragging. Past 40% width it completes (board slides off, peek settles, then the real route loads); otherwise both snap back. Telegram/iOS-style, not an instant jump. (Swipe-back always targets `/journals`, which is why the peek can be a fixed `<Home/>`.)
