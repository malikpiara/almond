...
- **Swipe-back is interactive with a destination peek** (`useSwipeBack`): a rightward drag **from anywhere on the screen** (not just the edge; commits on distance *or* a fast flick) drags the board with the finger via a direct `translateX`, while the **Journals screen parallaxes in behind it** — the board mounts `<Home/>` as a peek layer (`fixed inset-0 z-0`) only while dragging. Past 40% width it completes (board slides off, peek settles, then the real route loads); otherwise both snap back. Telegram/iOS-style, not an instant jump. (Swipe-back always targets `/journals`, which is why the peek can be a fixed `<Home/>`.)
...
- **Route transitions** use the **View Transitions API** (TanStack Router `viewTransition` + types), and are **platform-specific** because a full-width slide reads differently by screen size:
  - **Mobile** — directional slide with 30% parallax, mirroring the swipe-back. Forward (`slide-forward`) is snappy (180ms, `--ease-out-quint`, almost-instant); back (`slide-back`) is a touch slower (280ms, `--ease-out-cubic`) and reads as "closing".
